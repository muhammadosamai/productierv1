...
async function requireReleaseAccess(
  ...
  const release = await db.query.releases.findFirst({
    where: eq(releases.id, releaseId),
    columns: { id: true, productId: true, title: true, status: true },
  })
  if (!release) {
    set.status = 404
    return null
...
export const releaseRoutes = new Elysia({ prefix: '/api/releases' })
  ...
  .get('/:id', async ({ params: { id }, set, jwt, headers }) => {
    ...
    if (!access) return set.status === 404 ? notFound(set, 'Release not found') : forbidden(set)
    ...
    if (!release) return notFound(set, 'Release not found')
    return release
  })
  ...
  .put('/:id', async ({ params: { id }, body, set, jwt, headers }) => {
    ...
    if (!old) return notFound(set, 'Release not found')
  ...
  .delete('/:id', async ({ params: { id }, set, jwt, headers }) => {
    ...
    const [deleted] = await db.delete(releases)
      .where(eq(releases.id, id))
      .returning()
    if (!deleted) return notFound(set, 'Release not found')
  ...
  .post('/:id/deployments/:deploymentId/targets', async ({ params, body, jwt, headers, set }) => {
    ...
    if (!serverIds || serverIds.length === 0) {
      return badRequest(set, 'No server IDs provided')
    }
    ...
    logActivity({
      productId: access.release.productId,
      userName: user.name,
      userAvatar: user.avatar,
      userId: user.id,
      action: 'updated',
      entityType: 'release',
      entityId: params.id,
      entityTitle: access.release.title || `Release ${params.id}`,
      changes: [
        { field: 'deploymentId', from: null, to: params.deploymentId },
        { field: 'deploymentEnvironment', from: null, to: deployment.environment },
        { field: 'deploymentTargetsAdded', from: null, to: String(inserted.length) },
      ],
      routePathOverride: `/releases/${params.id}`,
    })
  ...
  .put('/:id/deployments/:deploymentId/targets/:targetId', async ({ params, body, jwt, headers, set }) => {
    ...
    const targetAction = body.status === 'failed' ? 'failed' : 'updated'
    logActivity({
      productId: access.release.productId,
      userName: user.name,
      userAvatar: user.avatar,
      ...
      changes: [
        { field: 'deploymentId', from: null, to: params.deploymentId },
        { field: 'deploymentTargetId', from: null, to: params.targetId },
        { field: 'deploymentEnvironment', from: null, to: deployment.environment },
        { field: 'status', from: target.status, to: body.status },
      ],
      routePathOverride: `/releases/${params.id}`,
    })
  ...
  .put('/:id/deployments/:deploymentId', async ({ params, body, jwt, headers, set }) => {
    ...
    const deploymentChanges: Array<{ field: string; from: string | null; to: string | null }> = [
      { field: 'deploymentId', from: null, to: params.deploymentId },
      { field: 'deploymentEnvironment', from: null, to: deployment.environment },
    ]
    if (body.status && body.status !== deployment.status) {
    ...
    if (body.notes !== undefined && String(body.notes ?? '') !== String(deployment.notes ?? '')) {
      deploymentChanges.push({
        field: 'deploymentNotes',
        from: deployment.notes ?? null,
        to: body.notes ?? null,
      })
    }
    if (deploymentChanges.length > 0) {
      logActivity({
        ...
        userAvatar: user.avatar,
        userId: user.id,
        action: body.status === 'failed' ? 'failed' : 'updated',
        entityType: 'release',
        entityId: params.id,
        entityTitle: access.release.title || `Release ${params.id}`,
  ...
  .delete('/:id/deployments/:deploymentId/targets/:targetId', async ({ params, jwt, headers, set }) => {
    ...
    logActivity({
      ...
      changes: [
        { field: 'deploymentId', from: null, to: params.deploymentId },
        { field: 'deploymentTargetId', from: params.targetId, to: null },
        { field: 'deploymentEnvironment', from: deployment.environment, to: deployment.environment },
      ],
      routePathOverride: `/releases/${params.id}`,
    })
